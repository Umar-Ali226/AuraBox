...
const ProductCards: React.FC = () => {
  ...
  const fetchCategories = async () => {
    ...
    setCategories(data.map((category: { title: string }) => category.title));
  };
...
<div className="flex justify-center items-center mt-12 mb-4">
  ...
  <button
    ...
    disabled={currentPage === 1}
    className={`px-[0.8rem] py-4 text-[0.785rem] font-semibold text-[#23A6F0] bg-white border-[0.1rem] border-gray-400 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
      currentPage === 1 ? "cursor-not-allowed opacity-50" : ""
    }`}
  >
    Previous
